...
//code that is run when playing the game
...
impl GameScene {
    ...
    pub fn from_stage(state: &mut SharedGameState, ctx: &mut Context, stage: Stage, id: usize) -> GameResult<Self> {
        ...
        //return OK with the contents of a successful initialization, populating this class' parameters
        Ok(Self {
            tick: 0,
            stage,
            water_params,
    ...
    //simply set the map name timeout variable to this value
    pub fn display_map_name(&mut self, ticks: u16) {
        self.map_name_counter = ticks;
    }

    //player has already been initialized, just make it live and snap it to P1
    ...
    //not sure what the popup is... is that the damage that appears above the NPC?
    ...
    //fade in/out bars? or is this something else?
    ...
    //... uh... ironhead not being drawn offscreen? (if this is the case, why only ironhead?)
    ...
    fn draw_light(&self, x: f32, y: f32, size: f32, color: (u8, u8, u8), batch: &mut Box<dyn SpriteBatch>) {
        ...
        batch.add_rect_scaled_tinted(
            ...
            size,
            size,
            &Rect::new(0, 0, 64, 64), //new rect, 64x64 square
        )
    }
                ...
                [(&self.player1, &self.inventory_player1), (&self.player2, &self.inventory_player2)].iter()
            {
                if player.cond.alive() && !player.cond.hidden() && inv.get_current_weapon().is_some() {
                    if state.settings.light_cone {
                        ...
                        //light cone will be different colors depending on the weapon held
                        ...
                        //draw a dot light on the player
                        self.draw_light(
                            interpolate_fix9_scale(
    ...
    //ripple the fancy water when an NPC is in it
                ...
                if npc.npc_flags.shootable() {
                    ...
                    if npc.life == 0 {
                        ...
                        if npc.shock < 14 {
                            ...
                            npc.shock = 16; //default NPC shock value
...
impl Scene for GameScene {
    fn init(&mut self, state: &mut SharedGameState, ctx: &mut Context) -> GameResult {
        ...
        //preprocessor command, inits lua scripting
        ...
        //create vector of NPC data, this means that the count of NPCs can be infinite
        let npcs = self.stage.load_npcs(&state.constants.base_paths, ctx)?;
        //for each in the NPC vector
        ...
        //pass stage textures to global state
        ...
        //control the map's lighting mode
    ...
    fn tick(&mut self, state: &mut SharedGameState, ctx: &mut Context) -> GameResult {
        ...
        if self.intro_mode {
            ...
            //go to title as soon as it hits the 9999 wait
        ...
        //pause menu related code
        
        //test: lock mouse cursor
        ctx.mouse_context.set_lock_state(!self.pause_menu.is_paused());
    ...
    fn draw(&self, state: &mut SharedGameState, ctx: &mut Context) -> GameResult {
        ...
        //should this be under tile layer: far foreground?
