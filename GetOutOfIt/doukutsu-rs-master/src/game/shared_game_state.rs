...
impl TimingMode {
    pub fn get_delta(self) -> usize {
        match self {
            ...
            //test: slow execution waaaay down
            TimingMode::_50Hz => 1000000000 / 50, //(1000000000 / 50) * 10,//1000000000 / 50,
...
impl SharedGameState {
    pub fn new(ctx: &mut Context) -> GameResult<SharedGameState> {
        ...
        //checks for unique version files in order to see what type of game version we have
        ...
        let more_rust = (current_time.month() == 7 && current_time.day() == 7) || settings.more_rust; //july 7 (it's my birthday!)
        ...
        //return a sharedGameState initialized with everything that we did above
        Ok(SharedGameState {
