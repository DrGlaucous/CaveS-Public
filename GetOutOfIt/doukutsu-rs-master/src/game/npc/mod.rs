...
bitfield! {
    ...
    impl Debug;
    /// Represented by 0x01
    pub solid_soft, set_solid_soft: 0; //set bit at location X
    /// Represented by 0x02
...
pub struct NPC {
    ...
    pub target_x: i32,
    pub target_y: i32,
    /// Previous X position, used by frame interpolator (nuevo: use this for high-speed collisions, too)
    pub prev_x: i32,
...
impl GameEntity<([&mut Player; 2], &NPCList, &mut Stage, &mut BulletManager, &mut Flash, &mut BossNPC, &Context)> for NPC {
    fn tick(
        &mut self,
        state: &mut SharedGameState,
        (players, npc_list, stage, bullet_manager, flash, boss, ctx): (
            [&mut Player; 2],
            &NPCList,
            &mut Stage,
            &mut BulletManager,
        ...
        //run the corresponding NPC code given the ID
        match self.npc_type {
        ...
        //show the damage value after the shock wears off
    ...
    fn draw(&self, state: &mut SharedGameState, ctx: &mut Context, frame: &Frame) -> GameResult {
        ...
        //hidden or dead: do not draw
        if !self.cond.alive() || self.cond.hidden() {
            return Ok(());
        }

        //pull in the texture for this NPC
        let texture_ref = state.npc_table.get_texture_ref(self.spritesheet_id);

        //load sprite sheet to refrence from
