...
impl StageData {
    ...
    pub fn load_stage_table(ctx: &mut Context, roots: &Vec<String>, is_switch: bool) -> GameResult<Vec<Self>> {
        ...
        //note: stage.sect is a raw chunk carved out of the EXE, no fancy processing done.
...
impl StageTexturePaths {
    ...
    //load in new stage paths when stage transferring
    pub fn update(&mut self, stage: &Stage) {
        
        //update background path
        self.background = stage.data.background.filename();

        //if pxpack_data from stage.data.pxpack_data is NOT null, then set each tileset path to the corresponding type from the stage data
        ...
        //otherwise, make all three paths the same "Stage/" + the name of the current loaded tileset
        else {
