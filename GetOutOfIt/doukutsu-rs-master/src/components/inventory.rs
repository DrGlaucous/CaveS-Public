...
impl GameEntity<(&mut Context, &mut Player, &mut Inventory, &mut HUD)> for InventoryUI {
    ...
    fn draw(&self, state: &mut SharedGameState, ctx: &mut Context, _frame: &Frame) -> GameResult<()> {
        ...
        //frame 0 is bright, frame 1 is dim
        //none will always use dim frames
        //if we're in weapons, the weapon_cursor_frame will be the result of self.tick
        //if we're in items, vice-versa
