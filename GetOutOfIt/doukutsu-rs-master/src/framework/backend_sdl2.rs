...
impl SDL2EventLoop {
    pub fn new(sdl: &Sdl, size_hint: (u16, u16), ctx: &Context) -> GameResult<Box<dyn BackendEventLoop>> {
        ...
        let mut win_builder = video.window("Getting Out of it with Dr. G (doukutsu-rs)", size_hint.0 as _, size_hint.1 as _);
...
impl BackendEventLoop for SDL2EventLoop {
    fn run(&mut self, game: &mut Game, ctx: &mut Context) {
        ...
        loop {
            ...
            ctx.mouse_context.update_mouse_coords(0, 0, state.scale); //default to 0 motion unless overriden by event
                ...
                match event {
                    ...
                    Event::Window { win_event, .. } => match win_event {
                        WindowEvent::FocusGained | WindowEvent::Shown => {
                            ...
                            ctx.mouse_context.set_lock_state(true);
                        }
                        WindowEvent::FocusLost | WindowEvent::Hidden => {
                            if state.settings.pause_on_focus_loss {
                                ...
                                *mutex = true;
                                ...
                                ctx.mouse_context.set_lock_state(false);
                            }
                        }
                    ...
                    Event::KeyDown { scancode: Some(scancode), repeat, keymod, .. } => {
                        if let Some(drs_scan) = conv_scancode(scancode) {
                            ...
                            //test: hide mouse if a is not the key being pressed
                            // let mut refs = self.refs.borrow_mut();
                            // let window = refs.window.window_mut();
                            // let mouse_util = window.subsystem().sdl().mouse();
                            // mouse_util.show_cursor(ctx.keyboard_context.is_key_pressed(ScanCode::A));
                        }
                    }
                    ...
                    {
                        ctx.mouse_context.update_mouse_coords(xrel, yrel, state.scale);   
                    }
        ...
        match &mut self.texture {
            ...
            Some(texture) => {
                ...
                for command in &self.commands {
                    //this is where we add new command types (like rotation handlers)
                    match command {
                        ...
                        //new, personalized command types
                        //source, destination, flip_x, flip_y, color_key, rotation, xloc, yloc (for rotation axis)
                        SpriteBatchCommand::DrawRectFlipTintedRotated(src, dest, flip_x, flip_y, color, rads, point_x, point_y, mag) =>
                        {
                            ...
                            let point = sdl2::rect::Point::new((*point_x / *mag )as i32, (*point_y / *mag) as i32);
