...
impl BackendEventLoop for GlutinEventLoop {
    fn run(&mut self, game: &mut Game, ctx: &mut Context) {
        ...
        let mut cum_pos = PhysicalPosition::new(0.0,0.0);
        ...
        event_loop.run(move |event, _, control_flow| {
            ...
            match event {
                ...
                Event::WindowEvent { event: WindowEvent::CursorMoved { device_id, position, modifiers }, window_id }
                    if window_id == window.window().id() =>
                {

                    //if locked, update relative position and snap to center of window
                    if ctx.mouse_context.get_lock_state()
                    {

                        let size = window.window().inner_size();
                        let newposition = PhysicalPosition::new(size.width / 2, size.height / 2);

                        //update realtive position
                        let delt_pos = PhysicalPosition::new(position.x - newposition.x as f64, position.y - newposition.y as f64);
                        
                        cum_pos.x += delt_pos.x;
                        cum_pos.y += delt_pos.y;
                        //ctx.mouse_context.update_mouse_coords(delt_pos.x as i32, delt_pos.y as i32, state_ref.scale); 

                        //re-center mouse
                        window.window().set_cursor_position(newposition);
                    ...
                    if window_id == window.window().id() =>
                {
                    if let Some(keycode) = input.virtual_keycode {
                        ...
                        //test: see if moving the mouse with a command is a self-feedback
                ...
                Event::MainEventsCleared => {
                    ...
                    //enable/disable relative mode (changed state may not be needed here...)
                    //ctx.mouse_context.update_mouse_coords(0, 0, state_ref.scale); //default to 0 motion unless overriden by event
                    
                    ctx.mouse_context.update_mouse_coords(cum_pos.x as i32, cum_pos.y as i32, state_ref.scale);
                    cum_pos.x = 0.0;
                    cum_pos.y = 0.0;

                    if(ctx.mouse_context.lock_state_changed())
                    {
                        ...
                        {
                            window.window().set_cursor_grab(false);
