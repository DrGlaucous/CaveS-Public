    ...
    for r in 0..64 {
        for c in 0..ch {
            ...
            if b & 0x20 != 0 {
                ...
                let _ = &data.push(note); size += 1;
                let _ = &data.push(ins); size += 1;
            }
            if b & 0x40 != 0 {
                let _ = &data.push(e.cmdlo); size += 1;
            }
            if b & 0x80 != 0 {
                let (cmd, info) = convert_cmd(e.cmd&0x0f, e.cmdlo);
                let _ = &data.push(cmd); size += 1;
                let _ = &data.push(info); size +=1;
            }
        }
        data.push(0); size += 1;
    }
