...
lazy_static! {
    static ref FT_ARPEGGIO_TABLE: Box<[u8; 32]> = Box::new([
          0,   1,   2,   0,   1,   2,   0,   1,
          2,   0,   1,   2,   0,   1,   2,   0,
    ...
    static ref FT_VIBRATO_TABLE: Box<[u8; 32]> = Box::new([
    ...
    static ref FT_PERIOD_TABLE: Box<[u16; 16*36]> = Box::new([
    // Tuning 0, Normal
...
impl FormatPlayer for FtPlayer {
    fn start(&mut self, data: &mut PlayerData, _mdata: &dyn ModuleData, mixer: &mut Mixer) {
