...
impl BossNPC {
    pub(crate) fn tick_b03_monster_x(
        ...
        match self.parts[0].action_num {
            ...
            1 => {
                ...
                self.parts[0].x = 200 * 0x200;
                self.parts[0].size = 3;
                self.parts[0].event_num = 1000;
                self.parts[0].hit_bounds =
                    Rect { top: 24 * 0x200, left: 24 * 0x200, bottom: 24 * 0x200, right: 24 * 0x200 };
                ...
                self.parts[3].size = 2;
                self.parts[3].target_y = 0;
                self.parts[3].display_bounds = Rect { top: 0x1000, left: 0x1000, bottom: 0x1000, right: 0x1000 };
                self.parts[3].hit_bounds =
                    Rect { top: 5 * 0x200, left: 5 * 0x200, bottom: 5 * 0x200, right: 5 * 0x200 };
                self.parts[3].npc_flags.set_ignore_solidity(true);

                self.parts[4] = self.parts[3].clone();
                self.parts[4].target_y = 1;

                self.parts[5] = self.parts[3].clone();
                self.parts[6] = self.parts[3].clone();
                self.parts[5].target_y = 2;
                self.parts[6].target_y = 3;
                ...
                self.parts[7].display_bounds =
                    Rect { top: 52 * 0x200, left: 24 * 0x200, bottom: 52 * 0x200, right: 24 * 0x200 };
                self.parts[7].hit_bounds = Rect { top: 0x1000, left: 24 * 0x200, bottom: 0x1000, right: 0x2000 };
                self.parts[7].npc_flags.set_ignore_solidity(true);
                self.hurt_sound[7] = 52;

                //track
                self.parts[9].cond.set_alive(true);
                self.parts[9].y = self.parts[0].y - 64 * 0x200;
                self.parts[9].x = self.parts[0].x - 56 * 0x200;
                self.parts[9].size = 3;
                self.parts[9].action_num = 0;
                self.parts[9].direction = Direction::Up;
                self.parts[9].display_bounds =
                    Rect { top: 36 * 0x200, right: 0x1000, bottom: 36 * 0x200, left: 24 * 0x200 };
                self.parts[9].hit_bounds = Rect { top: 28 * 0x200, left: 0x1000, bottom: 28 * 0x200, right: 0x2000 };
                ...
                self.parts[10].y = self.parts[0].y + 64 * 0x200;

                self.parts[11] = self.parts[9].clone();
                self.parts[11].y = self.parts[0].y - 64 * 0x200;
                self.parts[11].x = self.parts[0].x + 56 * 0x200;
                self.parts[11].direction = Direction::Bottom;
                self.parts[11].display_bounds.right = 24 * 0x200;
                self.parts[11].display_bounds.left = 0x1000;
                self.parts[11].hit_bounds.left = 0x2000;
                self.parts[11].hit_bounds.right = 0x1000;

                self.parts[12] = self.parts[11].clone();
                self.parts[12].y = self.parts[0].y + 64 * 0x200;

                //frame
                self.parts[13] = self.parts[9].clone();
                self.parts[13].display_bounds =
                    Rect { bottom: 30 * 0x200, left: 0x2000, top: 42 * 0x200, right: 0x2000 };
                ...
                self.parts[14].display_bounds.bottom = 42 * 0x200;
                self.parts[14].display_bounds.top = 30 * 0x200;

                self.parts[15] = self.parts[13].clone();
                self.parts[15].action_counter2 = 11;
                self.parts[15].anim_num = 2;
                self.parts[15].display_bounds.left = 0x2000;
                self.parts[15].display_bounds.right = 0x2000;

                self.parts[16] = self.parts[15].clone();
                self.parts[16].action_counter2 = 12;
                self.parts[16].anim_num = 3;
                self.parts[16].display_bounds.bottom = 42 * 0x200;
                self.parts[16].display_bounds.top = 30 * 0x200;
            }
            10 | 11 => {
                ...
                if self.parts[0].action_counter > 100 {
                    ...
                    self.parts[0].action_num = if self.parts[0].y > players[player_idx].y { 100 } else { 200 };
                }
            }
            100 | 101 => {
                ...
                let player_idx = self.parts[0].get_closest_player_idx_mut(&players);
                if self.parts[0].action_counter > 121 && players[player_idx].y > self.parts[0].y {
            ...
            600 | 601 => {
                if self.parts[0].action_num == 600 {
                    ...
                    self.parts[0].action_counter = 0;
                    self.parts[0].vel_x2 = self.parts[0].life as i32;
                    self.parts[1].action_num = 30;
                    self.parts[2].action_num = 30;
                }

                self.parts[0].action_counter += 1;
                if (self.parts[0].life as i32) < self.parts[0].vel_x2.saturating_sub(200)
            ...
            1000 => {
                ...
                let x = self.parts[0].y + self.parts[0].rng.range(-72..72) as i32 * 0x200;
                let y = self.parts[0].x + self.parts[0].rng.range(-64..64) as i32 * 0x200;
        ...
        self.parts[0].y +=
            (((self.parts[9].y + self.parts[10].y + self.parts[11].y + self.parts[12].y) / 4) - self.parts[0].y) / 16;
        ...
        match self.parts[i].action_num {
            ...
            10 | 11 => {
                if self.parts[i].action_num == 10 {
                    ...
                    self.parts[i].action_counter = (self.parts[i].target_y * 10 + 40) as u16;
    ...
    fn tick_b03_monster_x_track(&mut self, i: usize, state: &mut SharedGameState, players: &[&mut Player; 2]) {
        match self.parts[i].action_num {
            ...
            300 | 301 => {
                ...
                self.parts[i].vel_y  += 0x20;
                if self.parts[i].vel_y  > 0 {
                    self.parts[i].vel_y  = 0;
        ...
        if self.parts[i].action_num >= 100 && abs(players[player_idx].x - self.parts[i].x) < 0x800 {
        ...
        self.parts[i].vel_y  = clamp(self.parts[i].vel_y , -0x400, 0x400);
        self.parts[i].y += self.parts[i].vel_y ;
                ...
                if self.parts[i].action_counter > 0 {
                    ...
                    match self.parts[i].anim_num {
                        0 => {
                            ...
                            npc.y = self.parts[i].y + -30 * 0x200;
                            npc.x = self.parts[i].x + 0xc00;
                        }
                        1 => {
    ...
    fn tick_b03_monster_x_shield(&mut self, i: usize, state: &mut SharedGameState) {
        match self.parts[i].action_num {
            10 => {
                self.parts[i].target_y += 0x200;

                if self.parts[i].target_y > 32 * 0x200 {
                    self.parts[i].target_y = 32 * 0x200;
            ...
            20 => {
                ...
                if self.parts[i].target_y < 0 {
                    self.parts[i].target_y = 0;
                    self.parts[i].action_num = 0;
            ...
            30 => {
                ...
                if self.parts[i].target_y > 20 * 0x200 {
                    self.parts[i].target_y = 20 * 0x200;
                    self.parts[i].action_num = 0;
        ...
        //keep vector x here because we're still using left/right
        self.parts[i].y = self.parts[0].y + self.parts[i].direction.vector_x() * (24 * 0x200 + self.parts[i].target_y);
        self.parts[i].x = self.parts[0].x;
    ...
    fn tick_b03_monster_x_eye(
        ...
        match self.parts[i].action_num {
            ...
            10 | 11 => {
                ...
                if self.parts[i].action_counter > 0 {
                    ...
                    let player_idx = self.parts[i].get_closest_player_idx_mut(players);
                    let px = self.parts[i].y - players[player_idx].y;
                    let py = self.parts[i].x - players[player_idx].x;
                    ...
                    npc.vel_y  = (deg.cos() * -1536.0) as i32;
                    npc.vel_x  = (deg.sin() * -1536.0) as i32;
        ...
        match self.parts[i].target_y {
            0 => {
                self.parts[i].y = self.parts[0].y - 22 * 0x200;
                self.parts[i].x = self.parts[0].x - 0x2000;
            }
            1 => {
                self.parts[i].y = self.parts[0].y + 28 * 0x200;
                self.parts[i].x = self.parts[0].x - 0x2000;
            }
            2 => {
                self.parts[i].y = self.parts[0].y - 15 * 0x200;
                self.parts[i].x = self.parts[0].x + 14 * 0x200;
            }
            3 => {
                self.parts[i].y = self.parts[0].y + 17 * 0x200;
                self.parts[i].x = self.parts[0].x + 14 * 0x200;
            }
            _ => {}
        }
        ...
        self.parts[i].anim_rect = state.constants.npc.b03_monster_x
            [21 + self.parts[i].target_y as usize + 4 * self.parts[i].anim_num as usize];
    }
}
