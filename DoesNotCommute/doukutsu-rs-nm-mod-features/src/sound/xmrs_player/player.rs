//near-clone of the xmrsplayer.rs library, but broken out because I needed to change some things
//(If I were better at Rust, I could probably work with the existing library, but alas, I'm not)
//The original uses no-std, but that's not a problem with d-rs, so I'm going to bypass lifetime specifiers
//in favor of refCells

//use std::cell::RefCell;
use std::rc::Rc;

use crate::sound::xmrs_player::channel::Channel;
...
impl<'a> XmrsPlayerMod<'a> {
    pub fn new(module: Rc<Module>, sample_rate: f32, historical: bool) -> Self {
...
impl<'a> Iterator for XmrsPlayerMod<'a> {
    type Item = f32;
