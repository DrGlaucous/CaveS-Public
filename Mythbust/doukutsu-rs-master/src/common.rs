...
//is small enough to be inline (believe it is linear interlopation?)
#[inline(always)]
fn lerp_f64(v1: f64, v2: f64, t: f64) -> f64 {
    //percentage of original value + percentage of new value
    v1 * (1.0 - t) + v2 * t
}

pub fn interpolate_fix9_scale(old_val: i32, val: i32, frame_delta: f64) -> f32 {
    //if delta values is greater than 0x1800, or 12 pixels, 
    if abs(old_val - val) > 0x1800 {
        return val as f32 / 512.0; //will return the pixel count as float (guess time delta is large enough that we don't need to go into extra detail)
    }
    //go into extra detail
    unsafe {
        let interpolated = lerp_f64(old_val as f64, val as f64, frame_delta) as f32;
        let mag = G_MAG as f32; //magnification
