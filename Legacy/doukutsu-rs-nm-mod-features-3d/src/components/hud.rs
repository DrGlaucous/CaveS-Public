...
impl HUD {
    ...
    fn draw_old(&self, state: &mut SharedGameState, ctx: &mut Context, _frame: &Frame) -> GameResult {
        if !self.visible {
            return Ok(());
        }
    ...
    fn draw_new(&self, state: &mut SharedGameState, ctx: &mut Context, _frame: &Frame) -> GameResult {
        ...
        if !self.shock {
            ...
            if self.max_life != 0 {
                let bar_width = (self.life as f32 / self.max_life as f32 * 39.0) as u16;
                // heart/hp number box
                batch.add_rect(num_offset + 16.0, 16.0 + top, &Rect::new_size(0, 40, 24, 8));
                // life box
                batch.add_rect(bar_offset + 40.0, 16.0 + top, &Rect::new_size(24, 40, 40, 8));
                // life
                batch.add_rect(bar_offset + 40.0, 16.0 + top, &Rect::new_size(0, 24, bar_width, 8));
            }
        }
        ...
        if !self.shock {
            draw_number(num_offset + 40.0, 16.0 + top, self.life_bar as usize, Alignment::Right, state, ctx)?;
        }
...
impl GameEntity<(&Player, &mut Inventory)> for HUD {
    ...
    fn draw(&self, state: &mut SharedGameState, ctx: &mut Context, _frame: &Frame) -> GameResult {
        self.draw_new(state, ctx, _frame)?;
        //self.draw_old(state, ctx, _frame)?;
