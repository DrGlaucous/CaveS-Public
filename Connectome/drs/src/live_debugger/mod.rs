...
fn connectome_disp(ui: &imgui::Ui, npc: &NPC) {
    if let Some(connectome) = &npc.connectome {
        let weights = connectome.get_weight_list();

        //group text several per line
        ...
        for weight in weights {
            ...
            if idx > 9 {
                idx = 0;
                ui.text_wrapped(text.as_str());
                text = String::new();
            }
            idx += 1;
            text += format!("{:6}: {:6}\t", weight.0, weight.1.refrence_weight).as_str();
        }
        //draw leftovers
        ui.text_wrapped(text.as_str());
