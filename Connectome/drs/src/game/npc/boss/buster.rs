...
//extra NPCs that this boss uses (normal NPCs just put here for organization)
impl NPC {
    pub(crate) fn tick_n10899999_balfrog_projectile(&mut self, state: &mut SharedGameState) -> GameResult {
...
impl BossNPC
{
    ...
    //for refrence
    pub(crate) fn tick_b029999_balfrog(
        &mut self,
    ...
    //buster will spawn in on the first NPC it finds with event number 2000
    //his origin is 8 above the bottom center of his treads


    //buster's hands, head, and torso can be shot,
    //but ultimately, buster's torso is the only thing that matters

    //if the head is taken out, buster will flail his weapons
    //shooting the arms off disable them
    //enough hits to the torso and buster will be disabled


    /////////////////////////////
    //part list (draw order):
    //0: controller, never actually drawn or seen, manages all other parts
    //9: gun-hand
    //8: left forearm (rel. to buster)
    //7: treads
    //6: torso
    //5: head
    //4: right forearm
    ...
    )
    {
        ...
        match self.parts[0].action_num
        {
            0 => //init all sub-parts
            {
                //init controller
                {
                    let i_n = 0;
                    self.parts[i_n].action_num = 10;
                    self.parts[i_n].exp = 0;
                    self.parts[i_n].cond.set_alive(true);
                    self.parts[i_n].npc_flags.0 = 0;
                    self.parts[i_n].npc_flags.set_show_damage(true);
                    self.parts[i_n].npc_flags.set_event_when_killed(true);
                    self.parts[i_n].npc_flags.set_ignore_solidity(true);
                    //self.parts[i_n].npc_flags.set_invulnerable(true);
                    self.parts[i_n].life = 650;
                    self.hurt_sound[i_n] = 0;
                    self.parts[i_n].vel_x = 0;
                    self.parts[i_n].vel_y = 0;
                    self.parts[i_n].event_num = 2000;

                    //snap to NPC with event 2000
                    for npc in npc_list.iter()
                    {
                        if npc.event_num == self.parts[0].event_num
                        {
                            self.parts[0].x = npc.x;
                            self.parts[0].y = npc.y;
                            break;
                        }
                    }
                }
                ...
                //init treads
                {
                    ...
                    self.parts[i_n].npc_flags.set_ignore_solidity(true);
                    self.parts[i_n].npc_flags.set_invulnerable(true);
                    self.parts[i_n].npc_flags.set_solid_hard(true);
                    self.parts[i_n].cond.set_alive(true);

                    //offset from parent NPC (which is the controller)
                    self.parts[i_n].vel_x2 = 0 * 0x200;
                    self.parts[i_n].vel_y2 = 0 * 0x200;
                    self.parts[i_n].parent_id = 0; //self.parts[0].id; //set parent to controller

                    self.parts[i_n].anim_num = 18;

                    //method to call up parent and check if one exists:
                    //let parent = self.get_parent_ref_mut(npc_list);
                    //if let Some(parent) = parent {

                    self.parts[i_n].display_bounds =
                        Rect { left: 48 * 0x200, top: 16 * 0x200, right: 48 * 0x200, bottom: 16 * 0x200 };
                    self.parts[i_n].hit_bounds =
                        Rect { left: 48 * 0x200, top: 16 * 0x200, right: 48 * 0x200, bottom: 16 * 0x200 };
                ...
                //init torso
                {
                    ...
                    //self.parts[i_n].npc_flags.set_invulnerable(true);
                    self.parts[i_n].cond.set_alive(true);
                    self.parts[i_n].parent_id = 0; //set parent to controller

                    self.parts[i_n].anim_num = 2;
                    self.parts[i_n].vel_x2 = -4 * 0x200;
                    self.parts[i_n].vel_y2 = -48 * 0x200;

                    self.parts[i_n].display_bounds =
                        Rect { left: 28 * 0x200, top: 40 * 0x200, right: 28 * 0x200, bottom: 40 * 0x200 };
                    self.parts[i_n].hit_bounds =
                        Rect { left: 20 * 0x200, top: 40 * 0x200, right: 28 * 0x200, bottom: 40 * 0x200 };
                ...
                //init head
                {
                    ...
                    self.parts[i_n].anim_num = 0;
                    self.parts[i_n].vel_x2 = 0 * 0x200;
                    self.parts[i_n].vel_y2 = -92 * 0x200;

                    self.parts[i_n].display_bounds =
                        Rect { left: 16 * 0x200, top: 16 * 0x200, right: 16 * 0x200, bottom: 16 * 0x200 };
                    self.parts[i_n].hit_bounds =
                        Rect { left: 14 * 0x200, top: 12 * 0x200, right: 14 * 0x200, bottom: 16 * 0x200 };
                ...
                //left arm upper
                {
                    ...
                    self.parts[i_n].anim_num = 4;
                    self.parts[i_n].vel_x2 = 27 * 0x200;
                    self.parts[i_n].vel_y2 = -64 * 0x200;


                    self.parts[i_n].display_bounds =
                        Rect { left: 8 * 0x200, top: 8 * 0x200, right: 8 * 0x200, bottom: 32 * 0x200 };
                    self.parts[i_n].hit_bounds =
                        Rect { left: 8 * 0x200, top: 4 * 0x200, right: 8 * 0x200, bottom: 25 * 0x200 };


                    self.parts[i_n].anchor_x = (self.parts[i_n].display_bounds.left / 0x200) as f32;
                    self.parts[i_n].anchor_y = (self.parts[i_n].display_bounds.right / 0x200) as f32;
                ...
                //left arm lower (gun)
                {
                    ...
                    self.parts[i_n].npc_flags.set_invulnerable(true);
                    self.parts[i_n].cond.set_alive(true);
                    self.parts[i_n].parent_id = 8; //set parent to controller

                    self.parts[i_n].anim_num = 7;
                    self.parts[i_n].vel_x2 = 0 * 0x200;
                    self.parts[i_n].vel_y2 = 24 * 0x200;

                    self.parts[i_n].display_bounds =
                        Rect { left: 16 * 0x200, top: 8 * 0x200, right: 16 * 0x200, bottom: 40 * 0x200 };
                    self.parts[i_n].hit_bounds =
                        Rect { left: 8 * 0x200, top: 4 * 0x200, right: 8 * 0x200, bottom: 32 * 0x200 };

                    self.parts[i_n].anchor_x = (self.parts[i_n].display_bounds.left / 0x200) as f32;
                    self.parts[i_n].anchor_y = (self.parts[i_n].display_bounds.top / 0x200) as f32;
                ...
                {
                    let i_n = 4;
                    self.parts[i_n] = self.parts[8].clone();

                    self.parts[i_n].anim_num = 3;
                    self.parts[i_n].vel_x2 = -27 * 0x200;
                    self.parts[i_n].vel_y2 = -64 * 0x200;
                ...
                //right arm lower (saw)
                {
                    ...
                    //self.parts[i_n].npc_flags.set_invulnerable(true);
                    self.parts[i_n].cond.set_alive(true);
                    self.parts[i_n].parent_id = 4; //set parent to controller

                    self.parts[i_n].anim_num = 5;
                    self.parts[i_n].vel_x2 = 0 * 0x200;
                    self.parts[i_n].vel_y2 = 24 * 0x200;

                    self.parts[i_n].display_bounds =
                        Rect { left: 8 * 0x200, top: 8 * 0x200, right: 8 * 0x200, bottom: 64 * 0x200 };
                    self.parts[i_n].hit_bounds =
                        Rect { left: 8 * 0x200, top: 4 * 0x200, right: 8 * 0x200, bottom: 56 * 0x200 };
            ...
            _ =>
            {
                ...
                self.tick_b10_buster_treads(state, 7);
                self.tick_b10_buster_torso(state, 6);
                self.tick_b10_buster_head(state, player, 5);

                self.tick_b10_buster_forearm(state, player, 8);
                self.tick_b10_buster_gun_hand(state, player, 9);

                self.tick_b10_buster_forearm(state, player, 4);
                self.tick_b10_buster_saw_hand(state, player, 3);
    ...
    )
    {
        ...
        // }
        // self.parts[id].angle = core::f32::consts::PI * (((now % 100000) as f32)/ 2000.0); 


        self.parts[id].x = self.parts[id].vel_x2 + self.parts[self.parts[id].parent_id as usize].x;
        self.parts[id].y = self.parts[id].vel_y2 + self.parts[self.parts[id].parent_id as usize].y;

        self.parts[id].anim_rect = state.constants.npc.b10_buster[self.parts[id].anim_num as usize];
    }

    fn tick_b10_buster_saw_hand(
        &mut self,
        state: &mut SharedGameState,
        player: &Player, 
        id: usize,
    ...
    {
        self.parts[id].angle = -(PI / 4.0);

        let (off1, off2) = angle_rotate(self.parts[id].vel_x2 as f32,
            self.parts[id].vel_y2 as f32,
            self.parts[self.parts[id].parent_id as usize].angle);

        self.parts[id].x = (off1 as i32) + self.parts[self.parts[id].parent_id as usize].x;
        self.parts[id].y = (off2 as i32) + self.parts[self.parts[id].parent_id as usize].y;

        self.parts[id].anim_rect = state.constants.npc.b10_buster[self.parts[id].anim_num as usize];
    }

    fn tick_b10_buster_gun_hand(
        &mut self,
        state: &mut SharedGameState,
        player: &Player, 
        id: usize,
    )
    {
        ...
        self.parts[id].angle = core::f32::consts::PI * (((now % 100000) as f32)/ 1000.0); 


        //know where to put origin relative to previous segment
        let (off1, off2) = angle_rotate(self.parts[id].vel_x2 as f32,
                                    self.parts[id].vel_y2 as f32,
                                    self.parts[self.parts[id].parent_id as usize].angle);


        //self.parts[id].angle = -(PI / 2.0);
    ...
    )
    {
        ...
        if player.x < self.parts[id].x
        {
            //face left
            self.parts[id].anim_num = 0;
        }
        else
        {
            //right
            self.parts[id].anim_num = 1;   
        }
    ...
    fn tick_b10_buster_treads(
        &mut self,
        state: &mut SharedGameState,
        id: usize,
        //npc: &mut NPC, //if we just want the NPC ref, we cannot include self with this
    )
    {
        //can't do both (we need both)
        //let npc = &mut self.parts[id as usize];
        //let parent = &mut self.parts[self.parts[id].parent_id as usize];

        //animate treads
        self.parts[id].anim_counter += 1;
        if self.parts[id].anim_counter > 4
        {
            self.parts[id].anim_counter = 0;
            self.parts[id].anim_num += 1;

            if self.parts[id].anim_num > 19
            {
                self.parts[id].anim_num = 18;
            }
        }


        //we must refrence everything directly like this because rust doesn't like multiple mutable pointers
        //which I use to shorten names for readability
        self.parts[id].x = self.parts[id].vel_x2 + self.parts[self.parts[id].parent_id as usize].x;
        self.parts[id].y = self.parts[id].vel_y2 + self.parts[self.parts[id].parent_id as usize].y;
